...
const MISSING_API_KEY_MESSAGE =
  "AI API Key 未配置，请先在 Cloudflare Pages 环境变量中填写 AI_API_KEY。";
...
17. 用户输入的主题可能是一个带限制条件的细分主题。
18. 如果主题里包含限定词、筛选条件、形容词或范围，你必须严格按照这个条件生成词库。
19. 不要退回到大类词库。
20. 例如：用户输入“会飞的动物”，只能生成会飞的动物或会飞昆虫，例如：老鹰、麻雀、鸽子、燕子、乌鸦、蝙蝠、蜜蜂、蝴蝶、蜻蜓。不能生成老虎、河马、大象、猫、狗。
21. 例如：用户输入“水里的动物”，只能生成水生动物，不能生成陆地动物。
22. 例如：用户输入“两个字的动物”，只生成两个字的动物名称。
23. 例如：用户输入“带数字的成语”，只生成包含数字的成语。
24. 例如：用户输入“中国朝代”，只生成朝代名称，不能生成人物。
...
const getSafeErrorMessage = (status: number) => {
  ...
  if (status === 401 || status === 403) {
    return "AI API Key 无效或没有访问权限，请检查 Cloudflare Pages 环境变量 AI_API_KEY。";
  }
...
export const onRequest = async ({ request, env }: PagesFunctionContext) => {
  ...
  try {
    ...
    if (!response.ok) {
      return json({ error: getSafeErrorMessage(response.status) }, 500);
    }
